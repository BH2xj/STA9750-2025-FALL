...
The datasets were programmatically downloaded using the authenticated Kaggle API via command-line interface, ensuring reproducibility and avoiding manual browser-based access. The data were then imported and analyzed in R.

**Data Acquisition**

- Three datasets were obtained from Kaggle, an open data platform that hosts publicly available datasets related to social media usage, technology habits, and mental well-being.
- The datasets were downloaded using the Kaggle API, which allows programmatic access to Kaggle resources through authenticated requests.
- Prior to downloading the data, a Kaggle API token (kaggle.json) was generated from the user’s Kaggle account and securely configured in the local environment to enable API access.
- Each dataset was downloaded in compressed (.zip) format and subsequently unzipped locally using command-line tools.
- All extracted CSV files were organized into a dedicated project directory (data/final/) to maintain a clear and reproducible data structure.
- The datasets were then imported into R using the readr::read_csv() function with relative file paths, ensuring portability and reproducibility across different computing environments.
...
```{r, include=FALSE}
library(readr)
# Download the dataset zip file using Kaggle API

# Create file "final"
dir.create("data/final", recursive = TRUE)

# Load the CSV file into R
stress_wellness <- read_csv("data/final/Tech_Use_Stress_Wellness.csv")
mental_wellbeing <- read_csv("data/final/Tech_Gadget_Usage_and_Mental_Wellbeing.csv")
addiction <- read_csv("data/final/Students Social Media Addiction.csv")

```

**Data Processing**

While the overall analysis draws on all three sources, this individual report focuses on the **Tech Use & Stress Wellness** dataset to address **Specific Question 5**. To address SQ5, the dataset was filtered to include only variables directly related to social media use and mental health, including user ID, age, gender, social media usage hours, mental health score, and mindfulness minutes per day. A categorical Usage Group variable (low, moderate, high) was created based on social media usage levels to facilitate group comparisons. This focused preprocessing ensured alignment with SQ5 and supported the overall research question in conjunction with other sub-questions.

```{r, message=FALSE}
# Create usage groups (Low / Medium / High) and the subset
library(dplyr)
df <- stress_wellness|>
  select(
    user_id,
...
# Data analysis
## Refinement of Specific Question 5 (SQ5)
...
## Differences in Mental Health Scores Across Social Media Usage Levels
...
```
## Interaction Between Social Media Usage and Mindfulness Practice
